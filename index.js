...
function filterMessage(event) {
  ...
  const messageType = {
    ...
    image: handleImage(message, event.replyToken),
    video: handleVideo(message, event.replyToken),
    audio: handleAudio(message, event.replyToken),
    file: handleFile(message, event.replyToken),
    location: handleLocation(message, event.replyToken),
    sticker: handleSticker(message, event.replyToken),
    default: () => {
      throw new Error(`Unknown message: ${JSON.stringify(message)}`);
    },
  };
